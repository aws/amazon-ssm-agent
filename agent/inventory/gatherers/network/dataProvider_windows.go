...
// NetworkInterfaceConfiguration captures all relevant network information related to a network interface that we get
// from Win32_NetworkAdapterConfiguration class in windows.
//
...
// TODO: current implementation only allows 1 ipaddress, dns server address, gateway address per network interface
...
func EditNetworkData(log log.T, data inventory.NetworkData, cmdOutput string) (inventory.NetworkData, error) {
	...
	var dnsServerAddress, gatewayAddress, subnetMask string
