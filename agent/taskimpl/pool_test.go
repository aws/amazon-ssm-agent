...
func exercisePool(t *testing.T, pool task.Pool, jobID string, shouldCancel bool) {
	// submit job
	jobState := make(chan bool)
	var flag task.CancelFlag
	err := pool.Submit(logger, jobID, func(cancelFlag task.CancelFlag) {
		flag = cancelFlag
		jobState <- true
		if shouldCancel {
			assert.True(t, task.Canceled == cancelFlag.Wait())
		}
		jobState <- true
	})
	assert.Nil(t, err)
	...
	err = pool.Submit(logger, jobID, func(task.CancelFlag) {})
	assert.NotNil(t, err)
