...
func (p *Processor) processSendCommandMessage(context context.T,
	...
	go commandStateHelper.DeleteOldDocumentFolderLogs(log,
		newCmdState.DocumentInformation.InstanceID,
		context.AppConfig().Agent.OrchestrationRootDir,
		context.AppConfig().Ssm.RunCommandLogsRetentionDurationHours,
		isRunCommandLogFile,
		formRunCommandOrchestrationFolder)
...
// isRunCommandLogFile checks whether the file name format satisfies the format for RunCommand generated log files
func isRunCommandLogFile(fileName string) (matched bool) {
	matched, _ = regexp.MatchString("^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$", fileName)
	return
}

// formRunCommandOrchestrationFolder forms the orchestration dir name from the document state file
func formRunCommandOrchestrationFolder(documentStateFileName string) string {
