...
func TestValidateUpdate_EmptyTarget_ErrGetLatest(t *testing.T) {
	manifest := &updatemanifestmocks.T{}
	...
	manifest.On("GetLatestVersion", agentName).Return("", fmt.Errorf("SomeGetLatestError")).Once()

	noNeedToUpdate, err := util.ValidateUpdate(manifest, out, agentName, targetVersion, allowDowngrade)
	assert.NotNil(t, err)
	assert.Equal(t, "SomeGetLatestError", err.Error())
	assert.True(t, noNeedToUpdate)
	assert.Equal(t, contracts.ResultStatusInProgress, out.Status)
}

func TestValidateUpdate_EmptyTarget_FailParseDowngrade(t *testing.T) {
	manifest := &updatemanifestmocks.T{}
	out := &iohandler.DefaultIOHandler{
		Status: contracts.ResultStatusInProgress,
	}
	agentName := "agentName"
	targetVersion := ""
	allowDowngrade := "notfalsestring"

	util := &updateS3UtilImpl{}

	manifest.On("GetLatestVersion", agentName).Return("-1.-1", nil).Once()

	noNeedToUpdate, err := util.ValidateUpdate(manifest, out, agentName, targetVersion, allowDowngrade)
	assert.NotNil(t, err)
	assert.Contains(t, err.Error(), allowDowngrade)
	assert.True(t, noNeedToUpdate)
	assert.Equal(t, contracts.ResultStatusInProgress, out.Status)
}

func TestValidateUpdate_FailCompareVersion(t *testing.T) {
	manifest := &updatemanifestmocks.T{}
	out := &iohandler.DefaultIOHandler{
		Status: contracts.ResultStatusInProgress,
	}
...
func TestValidateUpdate_SameVersion(t *testing.T) {
	manifest := &updatemanifestmocks.T{}
	out := &iohandler.DefaultIOHandler{
		Status: contracts.ResultStatusInProgress,
	}
	...
	assert.Nil(t, err)
	assert.True(t, noNeedToUpdate)
	assert.Equal(t, contracts.ResultStatusSuccess, out.Status)
}

func TestValidateUpdate_NotAllowDowngrade(t *testing.T) {
	manifest := &updatemanifestmocks.T{}
	out := &iohandler.DefaultIOHandler{
		Status: contracts.ResultStatusInProgress,
	}
	currentVersion = "1.1.1.1"
	defer func() { currentVersion = version.Version }()

	targetVersion := "1.1.1.0"
	agentName := "agentName"
	allowDowngrade := "false"
	...
	noNeedToUpdate, err := util.ValidateUpdate(manifest, out, agentName, targetVersion, allowDowngrade)
	assert.NotNil(t, err)
	assert.Equal(t, "updating "+agentName+" to an older version, please enable allow downgrade to proceed", err.Error())
	assert.True(t, noNeedToUpdate)
	assert.Equal(t, contracts.ResultStatusInProgress, out.Status)
}

func TestValidateUpdate_FalseHasVersionTarget(t *testing.T) {
	manifest := &updatemanifestmocks.T{}
	out := &iohandler.DefaultIOHandler{
		Status: contracts.ResultStatusInProgress,
	}
	...
	manifest.On("HasVersion", agentName, targetVersion).Return(false).Once()

	noNeedToUpdate, err := util.ValidateUpdate(manifest, out, agentName, targetVersion, allowDowngrade)

	assert.NotNil(t, err)
	assert.Equal(t, agentName+" target version "+targetVersion+" is unsupported on current platform", err.Error())
	assert.True(t, noNeedToUpdate)
	assert.Equal(t, contracts.ResultStatusInProgress, out.Status)
}

func TestValidateUpdate_FalseHasVersionSource(t *testing.T) {
	manifest := &updatemanifestmocks.T{}
	out := &iohandler.DefaultIOHandler{
		Status: contracts.ResultStatusInProgress,
	}
	...
	manifest.On("HasVersion", agentName, targetVersion).Return(true).Once()
	manifest.On("HasVersion", agentName, currentVersion).Return(false).Once()

	noNeedToUpdate, err := util.ValidateUpdate(manifest, out, agentName, targetVersion, allowDowngrade)

	assert.NotNil(t, err)
	assert.Equal(t, agentName+" current version "+currentVersion+" is unsupported on current platform", err.Error())
	assert.True(t, noNeedToUpdate)
	assert.Equal(t, contracts.ResultStatusInProgress, out.Status)
}
...
func TestValidateUpdate_Success(t *testing.T) {
	...
	manifest.On("HasVersion", agentName, targetVersion).Return(true).Once()
	manifest.On("HasVersion", agentName, currentVersion).Return(true).Once()

	noNeedToUpdate, err := util.ValidateUpdate(manifest, out, agentName, targetVersion, allowDowngrade)

	assert.Nil(t, err)
	assert.False(t, noNeedToUpdate)
	assert.Equal(t, contracts.ResultStatusInProgress, out.Status)
}
