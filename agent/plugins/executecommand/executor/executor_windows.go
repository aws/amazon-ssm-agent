...
var commandPromptPath = filepath.Join(os.Getenv("SystemRoot"), "System32", "cmd.exe")

func populateCommand(filename string) (commandName string, commandArgs []string) {
	var cmdArgs = []string{"/k"}
	commandName = commandPromptPath
	commandArgs = cmdArgs
	return
}
...
func appendArgs(shellArgs, args []string, file string) (commandArgs []string) {
	...
	commandArgs = append(commandArgs, appconfig.ExitCodeTrap)
	return
}
